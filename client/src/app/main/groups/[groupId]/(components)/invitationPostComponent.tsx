...
export default function InvitationPostComponent({
    ...
    const { recordEventParticipation , getUserEventParticipatedByGroupId, userEventParticipated} = useUserGroupPostsStore();
    const eventsForGroup=getUserEventParticipatedByGroupId(groupId || '');
    ...
    const isRSVPed = useMemo(
        () => eventsForGroup.some(participation => participation.eventPostId === postId),
        [eventsForGroup, postId]
    );
    ...
    const handleRSVP = async () => {
        ...
        const response = await recordEventParticipation(user?.id || '', postId, groupId);
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 overflow-hidden">
            ...
            <div className="p-6 pb-4">
                ...
                <div className="flex justify-center mt-4">
                    <button
                        className={`px-8 py-3 rounded-lg font-medium text-sm transition-all duration-200 shadow-md hover:shadow-lg bg-teal-500 hover:bg-teal-600 text-white ${isRSVPed ? 'opacity-50 cursor-not-allowed' : ''} ${isRSVPed ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                        disabled={isRSVPed}

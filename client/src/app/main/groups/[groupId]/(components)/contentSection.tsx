...
export default function ContentSection({ groupId, groupName }: ContentSectionProps) {
    ...
    const posts = useMemo(() => userGroupPosts[groupId] || [], [userGroupPosts, groupId]);
    ...
    if (post.postType === 'Poll') {
        return <PollPostComponent key={post.postId} {...post} groupId={groupId}/>;
    }
    if (post.postType === 'Call for Volunteers') {
        return <VolunteerPostComponent key={post.postId} {...post} groupId={groupId}/>;
    }
    ...
    return null;
            ...
            <div className="fixed bottom-6 right-6 z-40">
                <button 
                    ...
                    className="bg-teal-700 hover:bg-teal-600 text-white rounded-full p-4 shadow-lg transition-colors duration-200 flex items-center justify-center"
                    title="Create Post"
                >
